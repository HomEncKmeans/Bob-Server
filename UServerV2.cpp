...
UServerV2::UServerV2(string u_serverIP, int u_serverPort, string t_serverIP, int t_serverPort, unsigned k, int max_round,
                 int variance_bound) {
    this->k = k;
    ...
    print(fhesiPubKey);
    print("CLIENT - UServerV2 SWITCH MATRIX ");
    print(keySwitchSI);
    this->socketAccept();
    ...
    while (r < this->max_round && s >= this->variance_bound) {
        ...
        for (auto &iter:this->A_r) {
            ...
            for (int i = 0; i < this->k; i++) {
                ...
                Ciphertext distance;
                distance = euclideanDistance(this->cipherpoints[iter.first],this->centroids[this->rev_centroids_clusters[i]],*this->client_SM);
...
void UServerV2::socketListen() {
    listen(this->u_serverSocket, 5);
    print("Server is listening...");

}

void UServerV2::socketAccept() {
    int socketFD;
...
void UServerV2::handleRequest(int socketFD) {
...
bool UServerV2::sendMessage(int socketFD, string message) {
...
string UServerV2::receiveMessage(int socketFD, int buffersize) {
...
ifstream UServerV2::receiveStream(int socketFD, string filename) {
    uint32_t size;
    auto *data = (char *) &size;
...
void UServerV2::log(int socket, string message) {
    sockaddr address;
...
void UServerV2::receiveEncryptionParamFromClient(int socketFD) {
...
void UServerV2::receiveEncryptedData(int socketFD) {
...
void UServerV2::initializeClusters() {
...
void UServerV2::initializeCentroids() {
    for (int i = 0; i < this->k; i++) {
        for (auto &iter : this->A) {
            if (iter.second[i] == 1) {
...
long UServerV2::calculateVariance() {
    int variance = 0;
    bitset<6> zeroset;
    for (auto &iter:this->A) {
...
void UServerV2::connectToTServer() {
    ...
    if (connect(this->t_serverSocket, (struct sockaddr *) &t_server_address, sizeof(t_server_address)) < 0) {
        ...
        print("UServerV2 CONNECTED TO TSERVER");
    }

}

ifstream UServerV2::distanceToStream(const Ciphertext &distance) {
    ofstream ofstream1("distance.dat");
    Export(ofstream1, distance);
    return ifstream("distance.dat");
}

void UServerV2::initializeKMToTServer() {
    this->connectToTServer();
...
ifstream UServerV2::centroidsCoefToStream(const Ciphertext &centroid) {
    ofstream ofstream1("centroid.dat");
    Export(ofstream1, centroid);
    return ifstream("centroid.dat");
}

void UServerV2::endKMToTserver() {
    this->connectToTServer();
...
void UServerV2::resultsToKClient() {
