...
UServerV3::UServerV3(string u_serverIP, int u_serverPort, string t_serverIP, int t_serverPort, unsigned k,
                     int max_round,
                     int variance_bound) {
    this->k = k;
    ...
    print(fhesiPubKey);
    print("CLIENT - UServerV3 SWITCH MATRIX ");
    print(keySwitchSI);
    this->socketAccept();
...
void UServerV3::socketListen() {
    listen(this->u_serverSocket, 5);
    print("Server is listening...");

}

void UServerV3::socketAccept() {
    int socketFD;
...
void UServerV3::handleRequest(int socketFD) {
...
bool UServerV3::sendMessage(int socketFD, string message) {
...
string UServerV3::receiveMessage(int socketFD, int buffersize) {
...
ifstream UServerV3::receiveStream(int socketFD, string filename) {
    uint32_t size;
    auto *data = (char *) &size;
...
void UServerV3::log(int socket, string message) {
    sockaddr address;
...
void UServerV3::receiveEncryptionParamFromClient(int socketFD) {
...
void UServerV3::receiveEncryptedData(int socketFD) {
...
void UServerV3::initializeClustersandCentroids() {
...
long UServerV3::calculateVariance() {
    Ciphertext variance;
    bool flag = true;
    for (auto &iter:this->A) {
        if (flag) {
...
void UServerV3::swapA() {
    this->A = this->A_r;
}

void UServerV3::connectToTServer() {
    ...
    if (connect(this->t_serverSocket, (struct sockaddr *) &t_server_address, sizeof(t_server_address)) < 0) {
        ...
        print("UServerV3 CONNECTED TO TSERVER");
    }

}

ifstream UServerV3::distanceToStream(const Ciphertext &distance) {
    ofstream ofstream1("distance.dat");
    Export(ofstream1, distance);
    return ifstream("distance.dat");
}

ifstream UServerV3::varianceToStream(const Ciphertext &distance) {
    ofstream ofstream1("variance.dat");
    Export(ofstream1, distance);
    return ifstream("variance.dat");
}

ifstream UServerV3::resultToStream(const Ciphertext &distance) {
    ofstream ofstream1("result.dat");
    Export(ofstream1, distance);
    return ifstream("result.dat");
}

void UServerV3::initializeKMToTServer() {
    this->connectToTServer();
...
ifstream UServerV3::centroidsCoefToStream(const Ciphertext &centroid) {
    ofstream ofstream1("centroid.dat");
    Export(ofstream1, centroid);
    return ifstream("centroid.dat");
}

void UServerV3::endKMToTserver() {
    this->connectToTServer();
...
void UServerV3::resultsToKClient() {
