...
class SingleCRT {
 ...
 // Without specifying a ZZX, we get the zero polynomial
  ...
  // all moduli used).  Using the given index set, the 
  // polynomial is reduced modulo the product of only the indexed primes.

  void toPoly(ZZX& p, const IndexSet& s) const;
  void toPoly(ZZX& p) const;
  
  friend ostream& operator<<(ostream &os, const SingleCRT &scrt) {
    ZZX p;
    scrt.toPoly(p);
    return (os << p);
  }

  void SetIndex(long index, const ZZX &poly);
